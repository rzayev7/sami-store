...
// Maps common colour names (RU + EN) → hex for the swatch dot
const COLOR_HEX = {
  // Russian
...
export default function ProductDetailClient({
  ...
  useEffect(() => {
    ...
    const isHydratedWithCurrentProduct =
      initialProduct && String(initialProduct?._id) === String(resolvedProductId);
    if (isHydratedWithCurrentProduct) {
              ...
              {Array.isArray(product.colors) && product.colors.length > 0 && (
                <div data-testid="color-selector">
                  <p className="mb-2.5 font-sans text-[10px] font-medium uppercase tracking-[0.2em] text-black/38 lg:text-start">
                    {t("product.colour")}
                    {selectedColor && (
                      <span className="ms-2 font-serif text-[13px] font-normal normal-case tracking-normal text-black/60">
                        {selectedColor}
                      </span>
                    )}
                  </p>
